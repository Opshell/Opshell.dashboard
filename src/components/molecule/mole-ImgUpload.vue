<script setup lang="ts">
    ...
    import { getData, iAxiosProgressEvent } from '@/hooks/useApi';
    ...
    // [M] - 上傳圖片 (暴露)
    const uploadImages = async (uploadUrl: string = '') => {
        if(uploadUrl !== ''){
            const headers = {
            ...
            return Promise.all(
                imageList.value.map(async (imageUploader: iImageUploader, i) => { // 一次傳
                    if (imageUploader.status === 'new') {
                        const formData = new FormData();

                        formData.append('images', imageUploader.fileData);

                        const onUploadProgress = (progressEvent: iAxiosProgressEvent) => {
                            ...
                            // if (progressEvent) {
                                ...
                                imageUploader.progress.loaded = (loaded >= imageUploader.progress.total)?
                        ...
                        return new Promise((resolve, reject) => {
                            getData(uploadUrl, 'POST', formData, headers, onUploadProgress) // 本機測試環境
                                .then((res) => {
                                    resolve(res?.data);
                                })
                                .catch((error) => {
        ...
        <label class="imgUploadBox" :class="{view: imageList.length > 0}" ref="uploadArea">
            ...
            >
                <ElImg v-for="(item, i) in imageList" :key="'previewImg_' + i"
                    :id="'previewImg_'+i" :src="item.preview" class="img"
